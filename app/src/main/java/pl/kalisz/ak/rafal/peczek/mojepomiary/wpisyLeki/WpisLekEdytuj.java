...
public class WpisLekEdytuj extends AppCompatActivity {

    public static final String EXTRA_Wpisu_ID = "wpisId";
    private String wpisId;
    private WpisLek wpisLek;

    private TextInputLayout wartoscOperacji, godzinaWykonania, dataWykonania;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_wpis_lek_edytuj);
        ...
        wpisLek = wpisLekRepository.findById(wpisId);
        ...
        Double wartoscObrotu = Double.parseDouble(wpisLek.getSumaObrotu());
        if(Double.parseDouble(wpisLek.getSumaObrotu())<0) {
            wartoscObrotu = wartoscObrotu *-1;
        }
        ...
        Double finalWartoscObrotu = wartoscObrotu;
            ...
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    for (QueryDocumentSnapshot queryDocumentSnapshot : task.getResult()) {
                        ...
                        if (wpisLek.getIdLeku().equals(lek.getId())) {
                            idWybranegoLeku = data.size() - 1;
                            ...
                            if (jednostka.getTypZmiennej() == 0) {
                                wartoscOperacji.getEditText().setText( finalWartoscObrotu.intValue() + "");
                                wartoscOperacji.getEditText().setInputType(InputType.TYPE_CLASS_NUMBER);
                            } else {
                                wartoscOperacji.getEditText().setText(finalWartoscObrotu +"");
                    ...
                    lek.setText(data.get(idWybranegoLeku));
                    ArrayAdapter adapter = new ArrayAdapter(WpisLekEdytuj.this, android.R.layout.simple_spinner_dropdown_item, data);
        ...
        Date date = wpisLek.getDataWykonania();
        SimpleDateFormat simpleTimeFormat = new SimpleDateFormat(getString(R.string.format_czasu));
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat(getString(R.string.format_daty));
        godzinaWykonania.getEditText().setText(simpleTimeFormat.format(date));
        dataWykonania.getEditText().setText(simpleDateFormat.format(date));
    ...
    public void aktualizujPozycie(View view) throws ParseException {
        ...
        if (validateData(wynik, idWybranegoLeku)) {
            ...
            wpisLekRepository.delete(wpisLek).addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                @Override
                public void onComplete(@NonNull Task<QuerySnapshot> task) {
                    if(task.isSuccessful()) {
                        wpisLekRepository.getQueryByLekId(lekId, 1).whereLessThan("dataWykonania", wpisLek.getDataWykonania()).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                            @Override
                            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                                if (task.isSuccessful()) {
                                    ...
                                    wpisLekRepository.insert(new WpisLek(doubleWynik.toString(), zapasLeku.toString(), lekId, FirebaseAuth.getInstance().getCurrentUser().getUid(), data, wpisLek.getDataUtwozenia(), new Date()));
                                }
                            }
                        });
                        finish();
                        ...
                        MaterialAlertDialogBuilder progresbilder = new MaterialAlertDialogBuilder(WpisLekEdytuj.this)
    ...
    public void usun(View view) throws ParseException {
        MaterialAlertDialogBuilder builder = new MaterialAlertDialogBuilder(WpisLekEdytuj.this);
